...
export async function generateStaticParams() {
  return getAllSlugs("vi").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "vi");
  if (!article) return {};
  ...
  meta.alternates = { canonical: `/vi/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { vi: `/vi/insights/${slug}` };
...
export default async function ViInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "vi");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "vi")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("vi")} {formatDate(frontmatter.dateModified, "vi")}</span>
              )}
              <span className="text-white/20">|</span>
