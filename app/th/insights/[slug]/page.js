...
export async function generateMetadata({ params }) {
  ...
  const article = getArticle(slug, "th");
  if (!article) return {};
  ...
  meta.alternates = { canonical: `/th/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { th: `/th/insights/${slug}` };
...
export default async function ThInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "th");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "th")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("th")} {formatDate(frontmatter.dateModified, "th")}</span>
              )}
              <span className="text-white/20">|</span>
