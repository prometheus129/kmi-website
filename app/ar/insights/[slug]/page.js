...
export async function generateStaticParams() {
  return getAllSlugs("ar").map((slug) => ({ slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  meta.alternates = { canonical: `/ar/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { ar: `/ar/insights/${slug}` };
...
export default async function ArInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "ar");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "ar")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("ar")} {formatDate(frontmatter.dateModified, "ar")}</span>
              )}
              <span className="text-white/20">|</span>
