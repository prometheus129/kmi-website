...
export async function generateStaticParams() {
  return getAllSlugs("bn").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "bn");
  if (!article) return {};
  ...
  meta.alternates = { canonical: `/bn/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { bn: `/bn/insights/${slug}` };
...
export default async function BnInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "bn");
    ...
    <div className="bg-navy min-h-screen text-white">
      ...
      <article className="pt-36 pb-8 lg:pt-44 lg:pb-12 px-6 lg:px-10">
        <div className="max-w-[780px] mx-auto">
          ...
          <PolymerCompassCTA locale="bn" variant={ctaVariant} />
        </div>
      </article>
