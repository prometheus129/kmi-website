...
export async function generateMetadata({ params }) {
  ...
  meta.alternates = { canonical: `/fr/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { fr: `/fr/insights/${slug}` };
...
export default async function FrInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "fr");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "fr")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("fr")} {formatDate(frontmatter.dateModified, "fr")}</span>
              )}
              <span className="text-white/20">|</span>
