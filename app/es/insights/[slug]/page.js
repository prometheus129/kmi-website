...
export async function generateMetadata({ params }) {
  ...
  meta.alternates = { canonical: `/es/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { es: `/es/insights/${slug}` };
...
export default async function EsInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "es");
        ...
        <div className="max-w-[780px] mx-auto">
          <RevealDiv>
            <nav className="flex items-center gap-2 text-xs text-muted mb-8 font-sans">
              <Link href="/es/insights" className="hover:text-teal transition-colors duration-200">
                Análisis
              </Link>
          ...
          <PolymerCompassCTA locale="es" variant={ctaVariant} />
        </div>
      </article>
