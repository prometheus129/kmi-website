...
export async function generateStaticParams() {
  return getAllSlugs("id").map((slug) => ({ slug }));
}
...
export async function generateMetadata({ params }) {
  ...
  meta.alternates = { canonical: `/id/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { id: `/id/insights/${slug}` };
...
export default async function IdInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "id");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "id")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("id")} {formatDate(frontmatter.dateModified, "id")}</span>
              )}
              <span className="text-white/20">|</span>
