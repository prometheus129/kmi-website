...
export async function generateStaticParams() {
  return getAllSlugs("ur").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "ur");
  if (!article) return {};
...
export default async function UrInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "ur");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "ur")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("ur")} {formatDate(frontmatter.dateModified, "ur")}</span>
              )}
              <span className="text-white/20">|</span>
