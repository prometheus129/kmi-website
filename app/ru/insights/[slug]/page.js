...
export async function generateStaticParams() {
  return getAllSlugs("ru").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "ru");
  if (!article) return {};
  ...
  meta.alternates = { canonical: `/ru/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { ru: `/ru/insights/${slug}` };
...
export default async function RuInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "ru");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "ru")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("ru")} {formatDate(frontmatter.dateModified, "ru")}</span>
              )}
              <span className="text-white/20">|</span>
