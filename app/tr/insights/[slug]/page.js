...
export async function generateStaticParams() {
  return getAllSlugs("tr").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "tr");
  if (!article) return {};
  ...
  meta.alternates = { canonical: `/tr/insights/${slug}` };

  if (article.translations.length > 0) {
    const languages = { tr: `/tr/insights/${slug}` };
...
export default async function TrInsightArticlePage({ params }) {
  const { slug } = await params;
  const article = getArticle(slug, "tr");
          ...
          <RevealDiv>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm text-muted mb-10 pb-10 border-b border-white/[0.08]">
              <span className="font-mono text-xs">
                {formatDate(frontmatter.date, "tr")}
              </span>
              {frontmatter.dateModified && frontmatter.dateModified !== frontmatter.date && (
                <span className="font-mono text-xs text-teal/70">· {getLastUpdatedLabel("tr")} {formatDate(frontmatter.dateModified, "tr")}</span>
              )}
              <span className="text-white/20">|</span>
