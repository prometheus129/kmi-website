...
Check at cpsc.gov/cgi-bin/labsearch/ — searchable by lab name or ID, and by rule. Search by the rule, not just the name — acceptance for one rule says nothing about another. Labs withdrawn for cause no longer appear as accepted, and CPSC publishes its adverse-action list alongside the search tool. For a fast first pass, the free [Lab & Test-Report Status Checker](/compliance/lab-status-checker) screens your lab against CPSC's for-cause withdrawals and walks you through this rule-specific check.
