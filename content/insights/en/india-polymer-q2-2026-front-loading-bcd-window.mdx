...
**Operating rule:** if your vessel has not departed China by around May 22, you are in the risk zone. After May 25-28, only short South China routes (Guangzhou/Nansha → Mundra) still have a plausible time margin, and only with direct service and no port congestion surprises.
...
**Practical guidance:** For Q2 allocation secured in the next two weeks, TT 30/70 is fastest for established suppliers. For new relationships where TT is not comfortable, open LC immediately — do not wait for optimal FOB. The calendar constraint is tighter than the price variance from waiting a week. For importers with cash-flow pressure but existing LC lines, UPAS extends the payment timeline without losing sight-payment benefit to the supplier.
...
**After May 22:**
- Time margin compressed — only short-transit routes (Guangzhou/Nansha → Mundra) practical
