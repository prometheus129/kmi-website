...
Two separate trade-remedy measures — one in force, one recommended and expected — now bracket the same product, calcium-carbonate filler masterbatch from Vietnam (HS 3824.99):
...
- **The IGST amplifies the cash hit but not the margin hit.** The duties sit inside the IGST base, so the residual channel pays about US$80 of IGST on a US$300 product — but for a GST-registered importer that IGST is creditable. The lasting P&L cost is the duty itself — US$144.19/MT, which turns a US$300 product on the water into a US$444 one on the books. The IGST on the duty is working capital, not margin — both matter, but to different people in your company.
- **Duty is now the largest single variable in the landed cost.** On the residual channel, ADD+CVD is 48% of the CFR price in this example. No freight negotiation, no payment-term concession, no supplier discount moves the needle the way the duty line does.
...
For a **residual-channel buyer**, the powder route has roughly US$144/MT of headroom to pay for compounding and still come out ahead — often decisive headroom, which is why the residual channel is where the powder question tends to answer itself. For an **An Tien named-channel buyer**, US$13.42 of headroom will not, on its own, fund a compounding step: the powder case there rests on other grounds — spec control, per-lot quality verification, supply-chain simplicity, insulation from future rate changes in the masterbatch measures — not on this year's duty math.
...
The full operational picture — powder handling, gravimetric dosing, dispersion, incoming QC — is in [the switching guide](/insights/india-switch-masterbatch-to-gcc-powder-compounding). And the duty structure points the market in exactly this direction — India was already the world's largest buyer of Vietnamese calcium carbonate before the measures — though the scale of the masterbatch-to-powder shift is still building rather than settled: the direction of travel, not yet a statistic.
