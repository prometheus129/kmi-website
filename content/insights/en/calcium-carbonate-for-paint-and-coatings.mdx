...
description: "How a paint formulator uses ground calcium carbonate as an extender pigment: how it spaces titanium dioxide to extend it (and where dry hiding takes over above the critical PVC), why its low oil absorption is an advantage, and how to pick the grade by target sheen — coarse for matte and primer, fine for gloss and enamel — plus whiteness and low iron for white and light-tint paint."
...
- **Titanium-dioxide spacing.** A fine extender physically separates the titanium dioxide particles so each one scatters light more efficiently — the paint-specific version of extending the pigment (below).
...
**Spacing — below the critical PVC.** Titanium dioxide is expensive, so formulators load it densely. But when the pigment particles crowd close together, their scattering fields overlap and each particle hides less efficiently — the paint is paying for pigment whose particles are interfering with each other. A **fine calcium carbonate acts as a spacer**: it separates the titanium dioxide particles so each one works closer to its full hiding power, and a portion of the pigment can then be removed while the paint stays within spec. The finer and more uniform the extender, the better it spaces — which is why ultrafine grades are the ones used to lift hiding efficiency.

**Dry hiding — above the critical PVC.** Every paint has a **critical pigment volume concentration (CPVC)** — the point where there is just enough binder to coat every particle and fill the gaps between them. Below it, the film is fully bound; above it, tiny **air voids** form between the particles. Those voids (refractive index about 1.0) sit far from the binder (about 1.5), so they scatter light strongly and add their own hiding — "dry hiding." This is why **flat, high-filler paints can carry a high load of calcium carbonate and less titanium dioxide** than a gloss paint: above the CPVC, the void structure does part of the hiding that pigment would otherwise do.
...
**Oil absorption — the parameter plastics buyers rarely meet.** In paint, how much binder an extender soaks up is a first-class number. It is measured as grams of oil per 100 grams of powder, and a high-oil-absorption extender demands more binder (which raises cost), thickens the paint, and lowers the CPVC. Here calcium carbonate has a genuine advantage: **its oil absorption is low** — well below clays, talc, or diatomaceous silica, which soak up several times more binder per unit weight. That means you can load more of it for less binder, the rheology stays manageable, and the CPVC stays higher. It is one of the honest reasons calcium carbonate is the most-used extender in paint. (Ask for the oil absorption figure on the technical data sheet — it varies with fineness, rising as the grade gets finer.)

## Choosing a grade by paint type and target sheen

This is where paint parts company with plastics most sharply. In plastics, finer is simply better within the gauge the product needs. In paint, **particle size sets the sheen**, so a formulator picks the grade to hit a target look — and coarser is often the right answer.
...
| Semi-gloss and gloss | Higher gloss | **KC-6** (~6 µm), or **KC-4** (~5.5 µm, D50 ~1.5 µm) for the highest gloss | A fine, tight top cut keeps the surface smooth so gloss survives, and a fine grade spaces the titanium dioxide best |

A note on gloss paint: it uses less extender overall, because it runs below the CPVC in a binder-rich film. When a fine calcium carbonate does go into a gloss or enamel, it is there to add volume and to space the titanium dioxide without pulling the gloss down — which a fine, tight grade can do and a coarse one cannot. For a true high-gloss enamel — where the surface is judged on a gloss meter — even a fine grade's top cut can register, so keep the extender loading low and confirm the gloss on your own drawdowns before you commit; that is exactly what the sample trial is for.
...
One scope note: calcium carbonate is used in both interior and exterior paint, but its surface is mildly acid-reactive, so exterior and other high-durability systems have to balance the filler loading against long-term chalking and washability. Treat this grade map as the starting point for interior architectural paint, and work the exterior durability trade-off out on your own line.
...
It is an extender pigment. It adds bulk and lowers cost, carries whiteness in a white or light paint, and — the part that pays for itself — a fine grade spaces the titanium dioxide particles so each one hides more efficiently, letting you remove part of the expensive pigment. In flat, high-filler paint above the critical PVC, the micro air-voids around the particles add their own hiding (dry hiding). It does not replace titanium dioxide's hiding power directly — its refractive index is too close to the dried binder's to scatter much light on its own.
