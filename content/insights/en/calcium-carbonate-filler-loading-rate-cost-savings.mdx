...
> **In short:** "Loading" is what percentage of **calpet (filler masterbatch)** you blend into resin — not the CaCO₃ percentage. Typical ranges: woven sacks **5–30%**, film **5–20%**, injection higher (verify per product). Raw-material saving per kilogram ≈ **loading × (1 − calpet price ÷ resin price)** — as an illustration, a 20–30% loading saves roughly **8–15% per kilogram**. But there is one important complication: because CaCO₃ is about **3× denser** than plastic, for products sold **per area or per length** (film, pipe), the per-unit saving is far smaller than the per-kilogram figure — unless you down-gauge.
...
- **Quality staying within limits.** The saving only materializes if tensile strength, dispersion, and surface quality stay within acceptable limits. Over-loading to save money can raise costs instead — through product rejects, customer complaints, and line downtime.

And there is one more, less obvious complication — the one that makes the per-kilogram numbers above misleading for many products.
...
| Per area / per sheet (density up ~12%) | close to zero — can even cost slightly more |

What happens: the ~8% per-kg saving is cancelled by needing ~12% more mass to cover the same film area — at this loading the per-area saving is essentially gone (and at a tight price gap, slightly negative). A wider calpet-to-resin price gap pulls it back toward break-even, but never restores the full 8% per-kg figure.
...
- **Processing and moisture control get harder.** High loading requires a matching resin MFI and draw ratio; on sacks, keep fine denier at ~15% or less. It also raises sensitivity to moisture and to dispersion defects — dry and store the filler properly, and expect line issues (die plate-out, bubble instability on film, screen-pack pressure on tape) to appear sooner. A coated grade helps with both.
...
*We supply [premium GCC (ground calcium carbonate) — the four-grade Kantor KC Series](/calcium-carbonate) — as powder and as calpet (filler masterbatch), coated and uncoated, high-whiteness (approaching 98%) — produced in Vietnam and ready to ship to Indonesia, alongside the China-origin resin you already import, handled as one relationship. Tell us the application, whiteness, and mesh you need.*
